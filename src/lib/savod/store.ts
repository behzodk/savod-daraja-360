...
import type { AssessmentResult, DemoProfileId, PassageWord } from "./types";
...
export const useSavod = create<SavodState>()(
  persist(
    (set) => ({
      ...
      saveResult: (r) => set((s) => ({ results: [r, ...s.results.filter((x) => x.id !== r.id)] })),
    }),
...
export function buildAssessmentFromProfile(
  draft: Pick<
    DraftAssessment,
    "demoProfileId" | "words" | "durationSeconds" | "wordsPerMinute" | "retellingTranscript"
  >,
  studentId: string,
  studentName: string,
  passageId: string,
  ...
  // Prefer real speech-to-text results captured during the reading stage; fall back
  // to the canned demo profile for stages that are still simulated.
  ...
  return {
    ...
    durationSeconds: draft.durationSeconds ?? profile.durationSeconds,
    wordsPerMinute: draft.wordsPerMinute ?? profile.wordsPerMinute,
    pauses: profile.pauses,
    words,
    retellingTranscript: draft.retellingTranscript ?? profile.retellingTranscript,
