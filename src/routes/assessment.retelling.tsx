...
function Retelling() {
  ...
  const [transcript, setTranscript] = useState(profile.retellingTranscript);
      ...
      {stage === "recording" && (
        <>
          ...
          <p className="text-muted-foreground mt-1 max-w-2xl">
            Har bir so‘zni eslash shart emas. Muhim voqealar va ular nima sababdan
            sodir bo‘lganini tushuntiring.
          </p>
      ...
      {stage === "processing" && (
        <div className="py-24 text-center">
          ...
          <h2 className="mt-6 font-display text-2xl font-bold">
            Qayta hikoya tahlil qilinmoqda
          </h2>
      ...
      {stage === "review" && (
        <>
          ...
          <div className="mt-6 rounded-xl border bg-warn-soft/60 p-4 text-sm">
            <span className="font-medium">Dastlabki kuzatuv:</span>{" "}
            voqealar eslab qolingan, lekin asosiy sabab-natija bog‘lanishi to‘liq
            aytilmagan.
          </div>
...
function Step({ done, children }: { done: boolean; children: React.ReactNode }) {
  return (
    ...
    >
      <span
        className={`h-2 w-2 rounded-full ${done ? "bg-success" : "bg-muted-foreground/40"}`}
      />
      {children}
    </li>
  );
